...
public class RenderMirrorManagerGeo extends GeoBlockRenderer<TileMirrorManager> {
    ...
    /**
     * Мягкое доворачивание угла по кратчайшему пути.
     */
    ...
    /**
     * Минимальный безопасный reset после renderItem:
     * - выключаем item lighting
     * - возвращаем прежний lightmap
     * - возвращаем цвет
     *
     * НИКАКИХ setActiveTexture / enableTexture2D / disableTexture2D тут нет.
     */
    private static void finishItemRender(float prevLightX, float prevLightY) {
    ...
    /**
     * Эмиссивный проход — максимально близко к безопасной схеме из requester.
     */
    ...
    public void render(TileMirrorManager te,
        ...
        // 2) активные зеркала + pending + flying items
        GlStateManager.pushMatrix();
        try {
            ...
            for (int i = 0; i < mirrors.size() && i < 24; i++) {
                ...
                float settleSpeed = Math.max(Math.abs(m.lastSpinStep) * DELIVERY_SPIN_ACCEL,
                        SPIN_BASE * DELIVERY_SPIN_ACCEL);
        ...
        // 3) эмисс в конце, как у requester
